  ...
  class CsvConverter
    ...
    def get_internet_archive_ids(internet_archive_file)
      ...
      CSV.foreach(internet_archive_file, headers: true) do |entry|
        unless entry['identifier']
          raise IaToHyacinth::UserError, "csv files MUST contain a field entitled 'identifier' formatted 'ldpd_########_000'"
        end
    ...
    def get_file_at_url(url)
      handle_http_error = proc do |exception|
        ...
        raise IaToHyacinth::UserError, "File not found: '#{url}'" if exception.message.downcase.include? '404 not found'
      end
    ...
    def get_marc_record(clio_id)
      ...
      MARC::Reader.decode(get_file_at_url(marc_file_path))
    end
    ...
    def print_id_from_subfield(prev, subfield, primary_clio_id)
      ...
      raise IaToHyacinth::UserError, "Multiple print records found for #{primary_clio_id}." if prev
    ...
    def get_print_record(record, primary_clio_id)
      ...
      raise IaToHyacinth::UserError, "Print recrd MARC file for id #{primary_clio_id} malformed or missing."
    end
    ...
    def get_clio_ids(csv_path)
      begin
        ...
        IaToHyacinth.logger.error(e.message)
        abort(e.message)
      end
      puts clio_ids
      clio_ids
    ...
    rescue IaToHyacinth::UserError => e
      IaToHyacinth.logger.error e.message
      nil
    rescue StandardError => e
      IaToHyacinth.logger.error e.message
      IaToHyacinth.logger.error e.backtrace.to_s
      raise
    end
    ...
    def convert_csv(internet_archive_file, output_file)
      ...
      JsonCsv.create_csv_for_json_records(output_file) do |csv_builder|
        clio_ids.each do |clio_id|
          hyacinth_record = clio_record_from_id clio_id
